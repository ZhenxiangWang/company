...
function mergeDataToChild(newData, _data, useDelete) {
  ...
  Object.keys(newData).forEach((key) => {
    if (typeof newData[key] === "object") {
      ...
      data[key] = mergeDataToChild(
        newData[key],
        deepCopy(data[key]) || (Array.isArray(newData[key]) ? [] : {}),
        useDelete
      );
    } else {
      data[key] = newData[key];
...
//     !obj1 ||
//     !obj2 ||
//     getEnumerableKeys(obj1).length !== getEnumerableKeys(obj2).length
//   ) {
//     return false;
//   }
...
//     const objA =
//       getEnumerableKeys($a).length > getEnumerableKeys($b).length ? $a : $b;
//     const objB =
//       getEnumerableKeys($a).length > getEnumerableKeys($b).length ? $b : $a;
...
const getParentRect = (item, parentData) => {
  ...
  function parentNode(parent) {
    ...
    if (
      i < 3 &&
      parent.parentNode &&
      parent.parentNode.tagName.toLocaleLowerCase() !== "body"
    ) {
